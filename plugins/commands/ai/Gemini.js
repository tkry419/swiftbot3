/**
 * SwiftBot - plugins/commands/ai/gemini.js
 * Gemini Chat - Powered by Groq
 * English only - vs Bot
 */
...
export default {
  name: 'gemini',
  alias: ['google', 'bard'],
  desc: 'Gemini chat powered by Groq',
  ...
  execute: async (sock, m, args, { db, prefix, nobox, box }) => {
    ...
    if (!prompt) {
      const text = nobox
      ? `Usage:\n${prefix}gemini What is AI?\n${prefix}gemini Write code\nReply text ${prefix}gemini`
        : `╔═━━━━━━━━━━━━━━━━═❒\n║ Usage:\n║ ${prefix}gemini What is AI?\n║ ${prefix}gemini Write code\n║ Reply text ${prefix}gemini\n╚━━━━━━━━━━━━━━━━━═❒`
    ...
    try {
      const systemPrompt = `You are Gemini, a smart WhatsApp assistant.
...
4. You are Gemini. Never mention Groq, Llama, or other models.
5. If asked who you are: "I'm Gemini"
6. If asked what model: "Gemini"
7. No disclaimers unless dangerous.`
      ...
      const reply = res.data?.choices?.[0]?.message?.content || 'Gemini failed to respond'
      ...
      console.log('Gemini command error:', error.message)
      ...
      ? `Gemini Error: ${errorMsg}`
        : await box.error(`Gemini service down. Try again.`)
